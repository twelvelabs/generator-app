...
const (
	ConfigPathDefault = ".{{ .ProjectDir }}.yml"
	ConfigPathEnv     = "{{ .ProjectEnvPrefix }}_CONFIG"
)

type Config struct {
	ConfigPath string
	Color      bool   `yaml:"color" env:"{{ .ProjectEnvPrefix }}_COLOR" default:"true"`
	Debug      bool   `yaml:"debug" env:"{{ .ProjectEnvPrefix }}_DEBUG"`
	Prompt     bool   `yaml:"prompt" env:"{{ .ProjectEnvPrefix }}_PROMPT" default:"true"`
	LogLevel   string `yaml:"log_level" env:"{{ .ProjectEnvPrefix }}_LOG_LEVEL" default:"warn" validate:"oneof=debug info warn error fatal"` //nolint: lll
}
